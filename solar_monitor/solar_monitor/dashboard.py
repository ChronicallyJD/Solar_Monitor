...
def render_victron_card(r: DeviceReading) -> str:
    ...
    if r.error and not partial:
        ...
        if r.device_type == "inverter":
            # Inverter card: DC battery input voltage, AC output watts, AC voltage
            # power_w holds ac_apparent_power (VA); display as W since PF unavailable
            body = (f'<div class="metrics">'
                    f'<div class="metric"><div class="metric-val v">{_fmt(r.voltage_v,2)}</div><div class="metric-lbl">DC In V</div></div>'
                    f'<div class="metric"><div class="metric-val w">{_fmt(r.ac_out_power_va,0)}</div><div class="metric-lbl">AC Out W</div></div>'
                    f'<div class="metric"><div class="metric-val pv">{_fmt(r.ac_out_voltage_v,1)}</div><div class="metric-lbl">AC V</div></div>'
                    f'</div><div class="state-row">'
                    f'<div class="state-kv"><span class="state-k">STATE</span><span class="state-v">{r.inverter_state or "—"}</span></div>'
                    f'<div class="state-kv"><span class="state-k">AC A</span><span class="state-v">{_fmt(r.ac_out_current_a,2)}</span></div>'
                    f'</div>{err_note}')
            ...
            body = (f'<div class="metrics">'
                    ...
                    f'<div class="state-kv"><span class="state-k">ERR</span><span class="state-v">{r.error_code if r.error_code is not None else "—"}</span></div>'
                    f'</div>{err_note}')
